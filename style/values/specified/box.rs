...
impl DisplayKeyword {
    fn parse<'i>(input: &mut Parser<'i, '_>) -> Result<Self, ParseError<'i>> {
        ...
        // Lynx's display property chooses one precomposed layout value:
        // none | contents | flex | grid | linear | relative, plus the four
        // atomic inline variants. It has no author-exposed flow layout and
        // rejects block/inline and all compound
        // <display-outside> <display-inside> forms. Unsupported public
        // constants and inside variants compile out with the parser.
...
impl Parse for Display {
    fn parse<'i, 't>(
        ...
        // Lynx does not expose flow layout to authors, but its UA sheet needs
        // one paragraph-forming box for the text component. Keep that entry
        // point origin-scoped rather than adding `block` to the public Lynx
        // grammar.
        if context.in_ua_sheet()
            && input
                .try_parse(|input| input.expect_ident_matching("block"))
                .is_ok()
        {
            return Ok(Display::internal_block());
        }
